...
cfg_if! {
    if #[cfg(feature = "force-rustls")] {
        ...
        fn assert_bad_hostname_error(err: &io::Error) {
            ...
            let err = err.downcast_ref::<native_tls_gmssl::Error>().unwrap();
...
async fn fetch_google() {
    ...
    let connector = tokio_tls_gmssl::TlsConnector::from(connector);
...
fn native2io(e: native_tls_gmssl::Error) -> io::Error {
