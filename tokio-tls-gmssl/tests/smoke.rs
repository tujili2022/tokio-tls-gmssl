...
use native_tls_gmssl::{Certificate, Identity};
use std::{fs, io::Error, path::PathBuf, process::Command};
use tokio::{
    io::{AsyncReadExt, AsyncWrite, AsyncWriteExt},
    net::{TcpListener, TcpStream},
};
use tokio_tls_gmssl::{TlsAcceptor, TlsConnector};
...
async fn client_to_server() {
    ...
    let server = async move {
        ...
        let native_tls_stream: &native_tls_gmssl::TlsStream<_> = socket.get_ref();
        let _peer_cert = native_tls_stream.peer_certificate().unwrap();
        let allow_std_stream: &tokio_tls_gmssl::AllowStd<_> = native_tls_stream.get_ref();
...
fn context() -> (TlsAcceptor, TlsConnector) {
    ...
    let acceptor = native_tls_gmssl::TlsAcceptor::builder(identity).build().unwrap();

    let cert = Certificate::from_der(&der).unwrap();
    let connector = native_tls_gmssl::TlsConnector::builder()
