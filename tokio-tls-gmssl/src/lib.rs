#![doc(html_root_url = "https://docs.rs/tokio-tls-gmssl/0.3.0")]
#![warn(
...
use native_tls_gmssl::{Error, HandshakeError, MidHandshakeTlsStream};
use std::fmt;
...
#[derive(Debug)]
pub struct TlsStream<S>(native_tls_gmssl::TlsStream<AllowStd<S>>);

/// A wrapper around a `native_tls_gmssl::TlsConnector`, providing an async `connect`
/// method.
#[derive(Clone)]
pub struct TlsConnector(native_tls_gmssl::TlsConnector);

/// A wrapper around a `native_tls_gmssl::TlsAcceptor`, providing an async `accept`
/// method.
#[derive(Clone)]
pub struct TlsAcceptor(native_tls_gmssl::TlsAcceptor);
...
impl<S> TlsStream<S> {
    ...
    where
        F: FnOnce(&mut native_tls_gmssl::TlsStream<AllowStd<S>>) -> io::Result<R>,
        AllowStd<S>: Read + Write,
    {
    ...
    /// Returns a shared reference to the inner stream.
    pub fn get_ref(&self) -> &native_tls_gmssl::TlsStream<AllowStd<S>> {
        &self.0
    }

    /// Returns a mutable reference to the inner stream.
    pub fn get_mut(&mut self) -> &mut native_tls_gmssl::TlsStream<AllowStd<S>> {
        &mut self.0
    }
}
...
where
    F: FnOnce(
            AllowStd<S>,
        ) -> Result<native_tls_gmssl::TlsStream<AllowStd<S>>, HandshakeError<AllowStd<S>>>
        + Unpin,
    S: AsyncRead + AsyncWrite + Unpin,
{
...
impl From<native_tls_gmssl::TlsConnector> for TlsConnector {
    fn from(inner: native_tls_gmssl::TlsConnector) -> TlsConnector {
        TlsConnector(inner)
    }
}
...
impl From<native_tls_gmssl::TlsAcceptor> for TlsAcceptor {
    fn from(inner: native_tls_gmssl::TlsAcceptor) -> TlsAcceptor {
        TlsAcceptor(inner)
    }
}
